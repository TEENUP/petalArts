...
def sign_up(request):
    if request.method == "POST":
        ...
        sponserId = "".join(random.choice(
            string.ascii_uppercase + string.digits) for x in range(0, 6))
        ...
        if isSponserIdValid and isUsernameValid and isPasswordValid:
            ...
            User.objects.create(sponserId=sponserId, username=username,
                                password=h_password, plan=product, amount=0.00)
            UserDetails.objects.create(username=username, firstName=firstName,
